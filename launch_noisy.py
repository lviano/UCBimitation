...
n_trajs = [2] if args.noiseE == 0.0 else [1]
expert_f = "noisy_trajs16.pkl" if args.noiseE == 0.0 else "noisy_trajs6.pkl" if args.noiseE == 0.05 else "noisy_trajs11.pkl"
for seed in range(0,5):
    for n in n_trajs:
        string = f" python imit_ucb/train_learner/{args.alg}.py \
            --env-name DiscreteGaussianGridworld-v0  --expert-trajs {expert_f} \
            --num-threads 1 --max-iter-num 200 \
