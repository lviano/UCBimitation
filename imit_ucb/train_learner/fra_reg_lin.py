...
if not os.path.isdir(assets_dir(subfolder+f"/frareg/learned_models")):
    os.makedirs(assets_dir(subfolder+f"/frareg/learned_models"))
if not os.path.isdir(assets_dir(subfolder+f"/frareg/reward_history")):
    os.makedirs(assets_dir(subfolder+f"/frareg/reward_history"))
...
def logsumexp(vec,axis):
    vec = np.exp(vec - np.max(vec,axis=axis,keepdims=True))
    return np.log(np.sum(vec,axis=axis))
def run_imitation_learning(K, eta=0.01):
    ...
    for k in range(K):
        ...
        V = 1/eta*logsumexp(eta*Q + np.log(policy),axis=1)
    ...
    with open(assets_dir(subfolder+f"/frareg/reward_history/{args.seed}_{args.n_expert_trajs}.p"), "wb") as f:
        pickle.dump(np.array(rs), f)
    with open(assets_dir(subfolder+f"/frareg/learned_models/{args.seed}_{args.n_expert_trajs}.p"), "wb") as f:
